...
class LogAnalyzer:
    ...
    def _extract_error_signature(self, message: str) -> str:
        ...
        important_words = [w for w in words if len(w) > 3 and w.lower() not in ['the', 'and', 'for', 'are', 'but', 'not']]
    ...
    async def update_model(self, logs: List[LogEntry]):
        ...
        try:
            ...
            messages = [log.message for log in logs]
            labels = [1 if log.log_level.upper() in ['ERROR', 'CRITICAL', 'FATAL'] else 0 for log in logs]
