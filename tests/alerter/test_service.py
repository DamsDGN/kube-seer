...
def anomaly(sample_timestamp):
    return Anomaly(
        ...
        description="CPU critical", score=0.92, details={},
...
def info_anomaly(sample_timestamp):
    return Anomaly(
        anomaly_id="a-003", source="metrics", severity=Severity.INFO,
        resource_type="node", resource_name="node-1", namespace="",
...
class TestAlerterServiceRouting:
    ...
    async def test_sends_to_alertmanager_primary(self, service, anomaly):
        ...
        result = AnalysisResult(anomalies=[anomaly], analysis_timestamp=anomaly.timestamp)
...
class TestAlerterServiceDedup:
    ...
    async def test_different_anomalies_not_deduped(self, service, sample_timestamp):
        a1 = Anomaly(
            anomaly_id="a-001", source="metrics", severity=Severity.CRITICAL,
            resource_type="node", resource_name="node-1", namespace="",
            description="CPU critical", score=0.9, details={}, timestamp=sample_timestamp,
        )
        a2 = Anomaly(
